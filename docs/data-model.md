...
(см. ADR-0032 §1). Материал может входить в один шаблон более одного
раза — уникальный индекс `(template_id, material_id)` снят
(см. ADR-0038); каждое добавление — независимая строка, фронтенд
подсвечивает повторяющиеся `material_id` как дубль, решение "оставить
оба или убрать один" — за человеком. `ProjectTemplateItem.material_id`
